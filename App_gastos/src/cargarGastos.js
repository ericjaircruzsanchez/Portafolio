//Importamos para formatear la fecha
import { format } from "date-fns";
//Convertir fecha de local storage
import { parseISO } from 'date-fns'
//Definimos el idioma
import { es } from 'date-fns/locale'
//Importamos mes para verificar la fecha
	...
	if (gastos && gastos.length >= 0) {
		...
		gastosDelMes.forEach((gasto) => {
			...
			<div class="gasto__info">
				...
				<p class="gasto__fecha">${format(parseISO(gasto.fecha), "d 'de' MMMM 'de' yyyy", /*Definimos el idioma invocando el metodo locale*/{ locale: es })}</p>
			</div>
		...
		//Si no hay gastos se activa el mensaje
		document
			.querySelector("#gastos .gastos__mensaje")
